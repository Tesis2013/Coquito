...
feature {NONE} -- Initialization
	...
	make_and_launch
		...
		do
			...
			l_box.extend_with_position_and_size (text_area, 300, 155, 465, 325)
	...
	load_description (a_item: EV_LIST_ITEM)
		...
		do
			...
			until
				l_file.after
			loop
				l_file.read_stream (128)
				text_area.append_text (l_file.last_string)
			end
		end
...
feature {ANY} -- Access
	...
	remove_reference_to_game_window (a_game_window: EV_WINDOW)
		...
		do
			if game_windows.has (a_game_window) then
				game_windows.start
				game_windows.search (a_game_window)
				game_windows.remove
			end
		end
	...
	Game_entries: attached LINKED_LIST [TUPLE [game, description: attached STRING; launcher: attached TYPE [LAUNCHER]]]
		...
		once
			create Result.make
				...
				-- Entries:
			Result.extend (["Seven Wonders", "group_1.txt", {SW_LAUNCHER}])
			Result.extend (["Citadels", "group_02.txt", {CD_LAUNCHER}])
			Result.extend (["Squatter", "group_3.txt", {SQ_LAUNCHER}])
			Result.extend (["Mag Blast", "group_4.txt", {MB_LAUNCHER}])
			Result.extend (["Scrabble", "group_5.txt", {I5_LAUNCHER}])
			Result.extend (["Monopoly", "group_6.txt", {G6_LAUNCHER}])
			Result.extend (["Dominion", "group_7.txt", {G7_LAUNCHER}])
			Result.extend (["Race for the Galaxy", "group_8.txt", {RG_LAUNCHER}])
			Result.extend (["Blokus", "group_09.txt", {G09_LAUNCHER}])
			Result.extend (["Truco", "group_10.txt", {TR_LAUNCHER}])
			Result.extend (["Hive", "group_11.txt", {HIVE_LAUNCHER}])
			Result.extend (["A Game of Thrones: The Card Game", "group_12.txt", {GT_LAUNCHER}])
			Result.extend (["TicTacToe", "ttt_description.txt", {TTT_LAUNCHER}])
			Result.extend (["Vision Demo", "vd_description.txt", {VD_LAUNCHER}])
		end
